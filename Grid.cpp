...
/*void Grid::SaveAll(ofstream& savedFile)
{
	int numOfLadders = 0;
	int numOfSnakes = 0;
	int numOfCards = 0;
	...
	int numOfGameObj[] = { numOfLadders, numOfSnakes, numOfCards };
	int currentGameObj = 0;

	for (int k=0; k<3; k++)
	{
		savedFile << numOfGameObj[currentGameObj] << endl;
		...
		for (int i = NumVerticalCells - 1; i >= 0; --i)
		{
			for (int j = 0; j < NumHorizontalCells; j++)
			{
				if (CellList[i][j]->GetGameObject() != NULL)
					CellList[i][j]->GetGameObject()->Save((GameObjectType)k, savedFile);
			}
		}
		currentGameObj++;
	}
...
void Grid::SaveAll(ofstream& savedFile, GameObjectType objType)
{
	for (int i = NumVerticalCells - 1; i >= 0; --i)
	{
		for (int j = 0; j < NumHorizontalCells; j++)
		{
			if (CellList[i][j]->GetGameObject() != NULL)
				CellList[i][j]->GetGameObject()->Save(objType, savedFile);
		}
	}
}
...
void Grid::ClearAll()
{
	for (int i = NumVerticalCells - 1; i >= 0; --i)
	{
		for (int j = 0; j < NumHorizontalCells; j++)
		{
			CellPosition currentCell(i, j);
			RemoveObjectFromCell(currentCell);
		}
	}
}

GameObject* Grid::Get_Object(const CellPosition& pos)
{
	int i = pos.HCell();
	int	j = pos.VCell();
	if (CellList[j][i]->GetGameObject())// searching For GameObject
		return CellList[j][i]->GetGameObject();
	else
...
Card* Grid::GetCard(CellPosition inputCellPosition)
{
	return CellList[inputCellPosition.VCell()][inputCellPosition.HCell()]->HasCard();
}
