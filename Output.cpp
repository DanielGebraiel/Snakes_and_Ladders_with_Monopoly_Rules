...
Output::Output()
{
	...
	UI.SnakelineWidth = 6;
	UI.SnakeColor = GREENYELLOW;

	// Colors of the 4 Players
	UI.PlayerColors[0] = PURPLE;
	UI.PlayerColors[1] = TURQUOISE;
	UI.PlayerColors[2] = HOTPINK;
...
int Output::GetCellStartX(const CellPosition& cellPos) const
{
	...
	return cellPos.HCell() * 110; // this line should be changed with your implementation
}
...
int Output::GetCellStartY(const CellPosition& cellPos) const
{
	...
	return (cellPos.VCell() * 55 + 55); // this line should be changed with your implementation
}
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_ADD_LADDER] = "images\\addladder.jpg";
	MenuItemImages[ITM_ADD_SNAKE] = "images\\addsnake.jpg";
	MenuItemImages[ITM_ADD_CARD] = "images\\addcard.jpg";
	MenuItemImages[ITM_COPY_CARD] = "images\\copycard.jpg";
	MenuItemImages[ITM_CUT_CARD] = "images\\cutcard.jpg";
	...
	MenuItemImages[ITM_SWITCH_TO_PLAY_MODE] = "images\\toplaymode.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_ROLL_DICE] = "images\\rolldice.jpg";
	MenuItemImages[ITM_INPUT_DICE_VALUE] = "images\\inputdicenumber.jpg";
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int num_of_lines = fromCell.VCell() - toCell.VCell();
	for (int i = 0; i < num_of_lines; ++i)
	{
		pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);
		pWind->DrawLine(x12, GetCellStartY(fromCell) - i * 55, x34, GetCellStartY(fromCell) - i * 55, FRAME);
	}
...
void Output::DrawSnake(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	int bottom_y = y1 + 2 * yChange;
	int middle_y = y1 + yChange;
	int left_x = x12 - xChange;
	int right_x = x12 + xChange;

	int x_array[] = { x12, right_x, x12, left_x };
	int y_array[] = { y1, middle_y , bottom_y, middle_y };
	///TODO: Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way
	pWind->DrawPolygon(x_array, y_array, 4, FILLED);
