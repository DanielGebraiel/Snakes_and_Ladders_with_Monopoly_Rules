...
CellPosition Input::GetCellClicked() const
{
	...
	if (UI.InterfaceMode == MODE_DESIGN)
	{
		if (y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			...
			//     
			int clickedHcell = (x / UI.CellWidth);
			int clickedVcell = ((y - UI.ToolBarHeight) / UI.CellHeight);

			cellPos.SetHCell(clickedHcell);
			cellPos.SetVCell(clickedVcell);
